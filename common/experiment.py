...
class Experiment(ABC):
    ...
    def build(self,
        ...
        for instance_values in tqdm(product(*ensemble_variables)): # 这个技巧可以的，*将列表解开成独立的参数（解压）
            instance_variables = dict(zip(variable_names, instance_values)) #打包为元组的列表，高级用法！
            instance_name = ','.join(['%s=%.4g' % (name, value) if isinstance(value, float) \
                                          else '%s=%s' % (name, str(value).replace(' ', '_')) \
                                      for name, value in instance_variables.items()])  # %-formatting is the OG of python formatting
            ...
            copy(self.config_path, instance_config_path) #文件拷贝
